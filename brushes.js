...
var DEFAULT_BRUSH = new Brush(function(ctx, e){
    var pmouseX = this.lastPoint[0],
            ...
            for (var i = 0;i<d;i+=BRUSH_SIZE/8){
                ...
                var action2 = {
                    func: ctx.fillRect,
                    params: [x-BRUSH_SIZE/2, y-BRUSH_SIZE/2, BRUSH_SIZE,BRUSH_SIZE]
                };
                this.currentStroke.addAction(action);
                this.currentStroke.addAction(action2);
  ...
  for (var i = 0, len = this.points.length; i < len; i++) {
    ...
    if (d < BRUSH_SIZE * 40) {
      var actions = [
        ...
        {
            func: ctx.lineTo,
            params: [this.points[i][0] - (dx * 0.2), this.points[i][1] - (dy * 0.2)]
        },
        {
            func: ctx.stroke,
            params: []
